...
def get_current_index(index_symbol = None) -> pd.DataFrame:
    """
    target website: https://quote.eastmoney.com/center/hszs.html
    支持上证指数、深证系列指数、指数成分、以及中证系列指数
    :param index_symbol: index symbol or None
    :rtype: str or None
    :return: a row with detailed data given individual index symbol, otherwise whole data if index symbol is none
...
def get_daily_index(symbol: str, start_date: str, end_date: str) -> pd.DataFrame:
    """
    东方财富网-中国股票指数-行情数据
...
if __name__ == "__main__":
    ...
    print(test_df)
    daily_df = get_daily_index(symbol='000300', start_date='19900101', end_date='20201230')
    print(daily_df)